...
SVGSpriteCss.prototype._init = function() {
	...
	this.data						= _.assign(this.data, {
		...
		spriteWidth					: (this.config.spriteMinSize && this.config.spriteMinSize.width) ? this.config.spriteMinSize.width : 0,
		spriteHeight				: (this.config.spriteMinSize && this.config.spriteMinSize.height) ? this.config.spriteMinSize.height : 0,
		spriteWidthEm				: 0,
		spriteHeightEm				: 0
	});
...
SVGSpriteCss.prototype._addShapeToSimpleCssSprite = function(shape, needsRegular, index, position) {
	...
	switch (this.config.layout) {
		...
		default:
			...
			this.data.spriteWidthEm	= this.data.spriteWidth / dimensions.width;
			this.data.spriteHeightEm	= this.data.spriteHeight / dimensions.height;
	}
